...
object MainApp {
  def main(args: Array[String]): Unit = {
    ...
    val corporateEquipAllocationListener = system.actorOf(Props(new CorporateEquipAllocation(
      system.actorOf(Props(new ManagerApprovalMessageListener())),
      system.actorOf(Props(new InventoryMessageListener())),
      system.actorOf(Props(new MaintenanceMessageListener())),
      system.actorOf(Props(new EmployeeMessageListener()))
    )))
